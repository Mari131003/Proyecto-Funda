...
# ----------- Bucle principal -----------
while True:
    patron = generar_patron(nivel) # Generar y mostrar patrón
    mostrar_patron(patron)
    tiempo_limite = {1: 1.0, 2: 0.5, 3: 0.2}[nivel] # Tiempo límite según nivel
    inicio = utime.ticks_ms()
    acierto = False
    while utime.ticks_diff(utime.ticks_ms(), inicio) < tiempo_limite * 1000: # Esperar respuesta del jugador
    ...
    if acierto:
        aciertos += 1
        if aciertos % 5 == 0: # Actualizar display cada 5 aciertos (1 set)
